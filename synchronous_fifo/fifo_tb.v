...
wire               full,empty,hf;
integer i,seed;
sync_fifo dut(.clk(clk),.wr(wr),.rd(rd),.rstn(rstn),.data_in(data_in),.data_out(data_out),.full(full),.empty(empty),.hf(hf));
...
if($test$plusargs("TEST_5")) begin
 //reset();
 dinR(5);
 data_read(8);   //it should not give the previous value?
 repeat(10) @(negedge clk);
end
if($test$plusargs("FULL_2")) begin
 reset();
 dinR(8);
 data_read(8);
 dinR(8);
...
if($test$plusargs("mix_read_write")) begin
 //reset();
 dinR(6);
 data_read(3);
 dinR(5);
